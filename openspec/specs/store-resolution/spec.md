...
A directory SHALL be treated as a store when it holds
`.openspec-store/store.yaml`, and the `id` in that file SHALL be the store's
name wherever the store is shown.

#### Scenario: A store found by the scanner
- **GIVEN** a discovered root holding `.openspec-store/store.yaml` with
  `id: nivis-tunnel`
- **WHEN** it is reported
- **THEN** it SHALL be reported as a store named `nivis-tunnel`, whatever its
  directory is called
...
act on the resolved root. None of them SHALL act on the starting directory when
the two differ.
